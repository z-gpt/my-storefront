...
const ShippingStatusCard = ({ slots, orderData, collapseThreshold = 0, }) => {
    ...
    return (<>
      {orderData?.shipments?.map(({ tracking, items, id }, index) => {
            ...
            {!isCompleted ? (<components_1.Accordion actionIconPosition="right" data-testid="dropinAccordion">
                <components_1.AccordionSection data-position={index + 1} defaultOpen={collapseThreshold >= items?.length} title={`Package contents (${items?.length} items)`}>
                  <components_1.ContentGrid maxColumns={6} emptyGridContent={<></>} className={'order-shipping-status-card__images'}>
                    {items?.map((item) => {
                        ...
                        return (<components_1.Image key={item.id} alt={label} src={url} width={85} height={114}/>);
                    })}
      ...
      {renderNotYetShippedProduct?.length ? (<components_1.Card variant="secondary" className="order-shipping-status-card" data-testid="dropinAccordionNotYetShipped2">
          ...
          <components_1.Accordion actionIconPosition="right">
            <components_1.AccordionSection title={`${translations.notYetShippedImagesTitle} ${renderNotYetShippedProduct?.length}`}>
              <div className="order-shipping-status-card__images">
                {renderNotYetShippedProduct?.map(({ id, thumbnail: { url, label } }) => (<components_1.Image key={id} alt={label} src={url} width={85} height={114}/>))}
              </div>
            </components_1.AccordionSection>

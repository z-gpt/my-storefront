...
describe('order/Components/ReturnsListContent', () => {
    ...
    const returnOrderList = mock_config_1.returnOrderListMock;
    const slots = {};
    ...
    test('renders minifiedView === false', () => {
        const { container } = (0, tests_1.render)(<ReturnsListContent_1.ReturnsListContent {...defaultProps} minifiedView={false} slots={{
                ReturnItemsDetails: () => { },
                DetailsActionParams: () => {
                    <a>Text button</a>;
                },
    ...
    test('renders without params', () => {
        const { container } = (0, tests_1.render)(<ReturnsListContent_1.ReturnsListContent withReturnsListButton={false} withHeader={false} withThumbnails={false} returnOrderList={returnOrderList.map((el) => ({ ...el, items: null }))} translations={translations} minifiedView={minifiedView} slots={{ ReturnItemsDetails: () => { } }} routeProductDetails={undefined}/>);
        const orderNumbers = tests_1.screen.getAllByText(/Order Number:/i);
        expect(!!container).toEqual(true);
        expect(orderNumbers).toHaveLength(1);
    });
    test('renders without default props', () => {
        const { container } = (0, tests_1.render)(<ReturnsListContent_1.ReturnsListContent returnOrderList={undefined} translations={undefined} minifiedView={undefined} withThumbnails={undefined} withHeader={undefined} returnsInMinifiedView={undefined} isMobile={undefined} withReturnsListButton={undefined} selectedPage={undefined} slots={{ ReturnItemsDetails: () => { } }}/>);
        expect(!!container).toEqual(true);
    });
});

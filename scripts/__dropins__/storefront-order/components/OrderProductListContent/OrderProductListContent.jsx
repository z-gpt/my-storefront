...
const OrderProductListContent = ({ loading, taxConfig, order = null, withHeader = true, showConfigurableOptions, }) => {
            ...
            return (<preact_1.Fragment key={index}>
              ...
              <ul className="order-order-product-list-content__items">
                {item.list?.map((product) => (<li data-testid="order-product-list-content-item" key={product.id}>
                    <components_2.CartSummaryItem loading={loading} product={product} itemType={item.type} taxConfig={taxConfig} translations={translations} showConfigurableOptions={showConfigurableOptions}/>
                  </li>))}
              </ul>
            </preact_1.Fragment>);
        })}

...
const CartSummaryItem = ({ loading, product, itemType, taxConfig, translations, showConfigurableOptions, onQuantity, }) => {
    ...
    return (<components_1.CartItem loading={loading} alert={isCancelled && inStock ? (<span>
          ...
          </span>) : (<></>)} configurations={showConfigurableOptions?.(configurations) ?? configurations} title={<div data-testid="product-name" className={(0, lib_1.classes)([
                'cart-summary-item__title',
                ['cart-summary-item__title--strikethrough', isCancelled],
            ])}>
          {product?.product?.name}
        </div>} sku={<div>{product?.product?.sku}</div>} quantity={product.totalQuantity} image={<components_1.Image src={product?.product?.thumbnail?.url ?? ''} alt={product?.product?.thumbnail?.label} loading="lazy" width="90" height="120"/>} {...configPrice} onQuantity={onQuantity}/>);
};

...
const ShippingStatus = ({ slots, className, collapseThreshold, orderData, }) => {
    const { order } = (0, hooks_1.useShippingStatus)({ orderData });
    return (<div className={(0, lib_1.classes)(['order-shipping-status', className])}>
      {order ? (<components_1.ShippingStatusCard slots={slots} orderData={order} collapseThreshold={collapseThreshold}/>) : (<components_1.CardLoader withCard={false}/>)}
    </div>);
};
